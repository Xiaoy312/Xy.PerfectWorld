...
namespace Xy.DataAnalysis.Legacy
{
    /// <summary>
    /// Borrowed from a previous project, PWH312, from 2010.
    /// Original source is unknown at this point. -xy 2015/06/21
    /// TODO : 
    ///     - export asm block into c++
    ///     - implement try catch block
    ///     - implement ROT for synchronous invocation
    /// </summary>
    public class ASM
    {
        ...
        public void Run(Core core)
        {
            int address;
            IntPtr hThread;
            byte[] opcodes = this.AsmChangebytes(this.Asmcode);
            
            address = API.VirtualAllocEx(core.Handle, 0, opcodes.Length, API.MEM_COMMIT, API.PAGE_EXECUTE_READWRITE);
            core.WriteBytes(address, opcodes);
            hThread = API.CreateRemoteThread(core.Handle, 0, 0, address, 0, 0, 0);
            if (hThread == IntPtr.Zero)
            {
                throw new Win32Exception();
            }

            API.WaitForSingleObject(hThread, uint.MaxValue);
            API.VirtualFreeEx(core.Handle, address, opcodes.Length, API.FreeType.Release);
